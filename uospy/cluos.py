...
from .keys import UOSKey, check_wif
from .utils import sig_digest
from .types import UOSEncoder, Transaction
import json

class Cluos :
    
    def __init__(self, url='http://43.242.156.61:9008', wallet_url='http://43.242.156.61:9008', version='v1') :
        ''' '''
        self._prod_url = url
    ...
    def get_actions(self, acct_name, pos=-1, offset=-20, timeout=30) :
        ...
        {"account_name":"uosnewyorkio","pos":-1,"offset":-20}
        '''
    ...
    def get_currency(self, code='uosio.token', symbol='UOS', timeout=30) :
        '''
        POST /v1/chain/get_currency_stats HTTP/1.0
        {"json":false,"code":"uosio.token","symbol":"UOS"}
        '''
        json={'json':False, 'code':code, 'symbol':symbol}
        return self.post('chain.get_currency_stats', params=None, json=json, timeout=timeout)

    def get_currency_balance(self, account, code='uosio.token', symbol='UOS', timeout=30) :
      '''
      POST /v1/chain/get_currency_balance HTTP/1.0
      {"account":"uosio","code":"uosio.token","symbol":"UOS"}
      '''
    ...
    def get_table(self, code, scope, table, table_key='', lower_bound='', upper_bound='', limit=10, timeout=30) :
        ...
        {"json":true,"code":"uosio","scope":"uosio","table":"producers","table_key":"","lower_bound":"","upper_bound":"","limit":10}
        '''
    ...
    def push_transaction(self, transaction, keys, broadcast=True, compression='none', timeout=30) :
        ...
        for key in keys :
            if check_wif(key) :
                k = UOSKey(key)
            elif isinstance(key, UOSKey) :
                k = key
            else :
                raise ValueError('Must pass a WIF string or UOSKey')
        ...
        data = json.dumps(final_trx, cls=UOSEncoder)
        if broadcast :
    ...
    def create_account(self, creator, creator_privkey, acct_name, owner_key, 
                       active_key='', stake_net='1.0000 SYS', stake_cpu='1.0000 SYS', ramkb=8, permission='active',
        ...
        newaccount_data = self.abi_json_to_bin('uosio', 'newaccount',{'creator' : creator, 'name' : acct_name, 'owner': owner_auth, 'active':active_auth})
        ...
        buyram_data = self.abi_json_to_bin('uosio', 'buyrambytes', {'payer':creator, 'receiver':acct_name, 'bytes': ramkb*1024})
        buyram_json = {
            'account' : 'uosio',
            'name' : 'buyrambytes',
            'authorization' : [
        ...
        # create delegatebw
        delegate_data = self.abi_json_to_bin('uosio', 'delegatebw',
